...
Если вы не знаете ID вашей админской группы:

1. Поставьте временное значение `ADMIN_CHAT_ID=1` в `.env`
2. Запустите бота: `python -m bot.main`
3. Напишите `/chatid` в нужной группе (бот должен быть в ней)
4. Бот ответит точным Chat ID группы
5. Пропишите полученный ID в `.env` как `ADMIN_CHAT_ID`
